...
grpc::Status FaissServer::Search(grpc::ServerContext* context,
  ...
  try {
    faissIndex->search(1, std_vector.data(), top_k, dist.data(), indices.data());
...
grpc::Status FaissServer::SearchById(grpc::ServerContext* context,
  ...
  std::vector<float> request_vector(dim);

  try {
    faissIndex->reconstruct(request_id, request_vector.data());
  } catch (std::exception& e) {
  ...
  try {
    faissIndex->search(1, request_vector.data(), top_k, dist.data(),
                   indices.data());
  } catch (std::exception& e) {
    logger_->error("Failed to search index: {}", e.what());
    return grpc::Status(grpc::StatusCode::INTERNAL, "Failed to search index");
